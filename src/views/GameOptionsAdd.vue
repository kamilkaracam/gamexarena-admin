...
<v-container fill-height fluid grid-list-xl>
    <v-layout justify-center wrap>
        <v-flex md12>
            <material-card color="green" title="Add New Game Option">
                <v-form ref="form" v-model="valid">
                    <v-container py-0>
                        <v-layout wrap row>
                            <v-flex xs12 md12>
                                <v-text-field v-model="formData.title" label="Game Option Name" required />
                            </v-flex>
                            <v-flex md5>
                                <v-btn small color="primary" v-on:click="submit">Create Game Option</v-btn>
