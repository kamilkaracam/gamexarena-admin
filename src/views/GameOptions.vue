...
<v-container fill-height fluid grid-list-xl>
    <v-layout justify-center wrap>
        <v-flex md12>
            <v-btn small color="primary" :to="'/game-options/' + $route.params.game_rule_id + '/new'">Add New Option</v-btn>
            <material-card color="green" title="Games" text="Games List">
                <v-data-table :headers="headers" :items="game_types" class="elevation-1">
                    <template slot="items" slot-scope="props">
                        <td>{{ props.item.rule_name }}</td>
                        <td>Rules ... </td>
                        <td>
                            <v-btn small color="primary" :to="'/game-options/' + props.item.id + '/edit'" >
                                <v-icon>edit</v-icon>
                            </v-btn>
